...
const CourseEditModal: React.FC<CourseEditModalProps> = ({
  ...
  const loadCategories = async () => {
    try {
      const cats = await courseManager.getCategories();
      setCategories(cats);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && course) {
        await courseManager.updateCourse(course.id, formData);
        setSuccess('课程更新成功！');
      } else {
        await courseManager.createCourse(formData);
        setSuccess('课程创建成功！');
      }
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm('确定要删除这个课程吗？此操作不可撤销。')) {
      setIsLoading(true);
      try {
        await courseManager.deleteCourse(course.id);
        setSuccess('课程删除成功！');
        setTimeout(() => {
          onSuccess();
          onClose();
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-emerald-500"
                  >
                    <option value="beginner" className="bg-slate-800">初级</option>
                    <option value="intermediate" className="bg-slate-800">中级</option>
                    <option value="advanced" className="bg-slate-800">高级</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <div className="w-full px-4 py-2 bg-white/5 border border-white/20 rounded-lg text-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-emerald-500"
                  >
                    <option value="draft" className="bg-slate-800">草稿</option>
                    <option value="published" className="bg-slate-800">已发布</option>
                    <option value="archived" className="bg-slate-800">已归档</option>
                  </select>
                </div>
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center">
                      ...
                      />
                      <span className="ml-2 text-sm text-gray-300">免费课程</span>
                    </label>
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4 text-emerald-500 bg-white/10 border-white/20 rounded focus:ring-emerald-500 focus:ring-2"
                      />
                      <span className="ml-2 text-sm text-gray-300">热门课程</span>
                    </label>
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4 text-emerald-500 bg-white/10 border-white/20 rounded focus:ring-emerald-500 focus:ring-2"
                      />
                      <span className="ml-2 text-sm text-gray-300">新课标签</span>
                    </label>
                  </div>
                </div>
              </div>
