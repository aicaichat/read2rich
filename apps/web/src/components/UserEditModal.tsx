...
import { 
  ...
  Phone, Shield, Crown, Star, Users,
  AlertCircle, CheckCircle, Eye, EyeOff,
  Globe, Github, MessageCircle
} from 'lucide-react';
import Button from './ui/Button';
import { User as UserType, UserFormData, userManager } from '../lib/user-management';
...
const UserEditModal: React.FC<UserEditModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && user) {
        await userManager.updateUser(user.id, formData);
        setSuccess('用户信息更新成功！');
      } else {
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case 'instructor': return <Crown className="w-4 h-4" />;
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  >
                    <option value="student" className="bg-slate-800">学生</option>
                    <option value="instructor" className="bg-slate-800">讲师</option>
                    <option value="admin" className="bg-slate-800">管理员</option>
                    <option value="guest" className="bg-slate-800">访客</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  >
                    ...
                    <option value="suspended" className="bg-slate-800">已暂停</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-emerald-500"
                  >
                    <option value="zh-CN" className="bg-slate-800">中文</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <input
                      ...
                      onChange={(e) => handleSocialLinkChange('wechat', e.target.value)}
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <input
                      ...
                      onChange={(e) => handleSocialLinkChange('qq', e.target.value)}
                      className="flex-1 px-4 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-emerald-500"
                      placeholder="QQ"
                    />
                  </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <label className="flex items-center space-x-2">
                    ...
                    />
                    <span className="text-gray-300 text-sm">邮件通知</span>
                  </label>
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      ...
                      className="w-4 h-4 text-emerald-500 bg-white/10 border-white/20 rounded focus:ring-emerald-500"
                    />
                    <span className="text-gray-300 text-sm">短信通知</span>
                  </label>
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      ...
                      className="w-4 h-4 text-emerald-500 bg-white/10 border-white/20 rounded focus:ring-emerald-500"
                    />
                    <span className="text-gray-300 text-sm">推送通知</span>
                  </label>
                </div>
              </div>
